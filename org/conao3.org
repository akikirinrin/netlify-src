...
# (let ((sha1 (string-trim (shell-command-to-string "head -c 1k /dev/random | openssl sha1 | awk '{print $2}'"))))
#   (cl-loop for (a b c d) on (cdr (split-string sha1 "")) by #'cddddr
...
その考察は次の記事「[[誕生日のパラドックスから考察する、ブログURLに必要なランダムビット数]]」に譲るとして、この記事ではパーマリンク設計について大きな決定ができたので、とても満足しています。
...
** DONE 誕生日のパラドックスから考察する、ブログURLに必要なランダムビット数
...
3. $P_1$ と $P_2$ が $50$ %を超えるのにそれぞれ必要な人数 $n_\text{min1}$, $n_\text{min2}$
   ...
   #+begin_src python :exports both :session py1 :cache yes :file ../static/files/65d26e9c-99a3-411c-9dbf-047dcd474d5c.png
...
次の記事「」ではこのフォーマットを実現するためのox-hugoの運用について書きたいと思います。
...
** TODO ox-hugo用のorg-captureテンプレートについて
:PROPERTIES:
:EXPORT_FILE_NAME: 2020-1049-5b91
:EXPORT_HUGO_TAGS: org ox-hugo org-capture permalink meta
:EXPORT_HUGO_LASTMOD:
:END:
*** 
